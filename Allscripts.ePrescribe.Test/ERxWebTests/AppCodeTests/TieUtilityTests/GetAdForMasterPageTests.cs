...
{
    [TestClass]
    public class GetAdForMasterPageTests
    {
        [TestMethod]
        public void should_use_pageName_from_absoluteUrlParser_to_get_location_name()
        {
            ...
            //act
            TieUtility.GetAdForMasterPage("", new HttpCookieCollection(), tieUtilityMock);

            //assert
            var arguments = tieUtilityMock.GetArgumentsForCallsMadeOn(x => x.GetMasterPageTieLocationName(Arg<string>.Is.Anything));
        ...
        public void should_use_locationName_from_tieLocation_method_to_get_ad_placement()
        {
            //arrange
            var tieUtilityMock = MockRepository.GenerateMock<ITieUtility>();
            tieUtilityMock.Stub(x => x.GetMasterPageTieLocationName("")).IgnoreArguments().Return(Constants.TIELocationPage.Login_Page);
